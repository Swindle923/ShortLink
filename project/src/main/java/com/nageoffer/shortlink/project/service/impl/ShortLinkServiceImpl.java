...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    public ShortLinkCreateRespDTO createShortLinkByLock(ShortLinkCreateReqDTO requestParam) {
        ...
        try {
            ...
            try {
                ...
                bizMetricsKit.increment("shortlink.biz.shortlink.create", "result", "duplicate", "mode", "lock");
                throw new ServiceException(String.format("短链接：%s 生成重复", fullShortUrl));
            }
            stringRedisTemplate.opsForValue().set(
                    String.format(GOTO_SHORT_LINK_KEY, fullShortUrl),
        ...
        bizMetricsKit.increment("shortlink.biz.shortlink.create", "result", "success", "mode", "lock");
        return ShortLinkCreateRespDTO.builder()
    ...
    public ShortLinkBatchCreateRespDTO batchCreateShortLink(ShortLinkBatchCreateReqDTO requestParam) {
        ...
        for (int i = 0; i < originUrls.size(); i++) {
            ...
            try {
                ...
                bizMetricsKit.increment("shortlink.biz.shortlink.batch_create.item", "result", "fail");
            }
        }
        bizMetricsKit.increment("shortlink.biz.shortlink.batch_create.item", result.size(), "result", "success");
    ...
    public void updateShortLink(ShortLinkUpdateReqDTO requestParam) {
        ...
        if (Objects.equals(hasShortLinkDO.getGid(), requestParam.getGid())) {
            ...
            try {
                ...
                ShortLinkDO shortLinkDO = ShortLinkDO.builder()
                        ...
                        .maxAccessCount(normalizeMaxAccessCount(requestParam.getMaxAccessCount()))
                        .currentAccessCount(hasShortLinkDO.getCurrentAccessCount())
        ...
        bizMetricsKit.increment("shortlink.biz.shortlink.update", "result", "success");
    }
    ...
    private Integer normalizeMaxAccessCount(Integer maxAccessCount) {
    ...
    private boolean tryConsumeAccessQuota(String fullShortUrl) {
        LambdaQueryWrapper<ShortLinkGotoDO> linkGotoQueryWrapper = Wrappers.lambdaQuery(ShortLinkGotoDO.class)
                .eq(ShortLinkGotoDO::getFullShortUrl, fullShortUrl);
        ShortLinkGotoDO shortLinkGotoDO = shortLinkGotoMapper.selectOne(linkGotoQueryWrapper);
        if (shortLinkGotoDO == null) {
        ...
        return tryConsumeAccessQuota(shortLinkGotoDO.getGid(), fullShortUrl);
    }

    private boolean tryConsumeAccessQuota(ShortLinkDO shortLinkDO) {
        return tryConsumeAccessQuota(shortLinkDO.getGid(), shortLinkDO.getFullShortUrl());
    }
    ...
    private void verificationWhitelist(String originUrl) {
        ...
        if (StrUtil.isBlank(domain)) {
            bizMetricsKit.increment("shortlink.biz.whitelist.reject", "reason", "invalid_domain");
            throw new ClientException("跳转链接填写错误");
        }
        List<String> details = gotoDomainWhiteListConfiguration.getDetails();
        if (!details.contains(domain)) {
            bizMetricsKit.increment("shortlink.biz.whitelist.reject", "reason", "not_in_whitelist");
            throw new ClientException("演示环境为避免恶意攻击，请生成以下网站跳转链接：" + gotoDomainWhiteListConfiguration.getNames());
        }
        bizMetricsKit.increment("shortlink.biz.whitelist.allow", "result", "pass");
    }
}
